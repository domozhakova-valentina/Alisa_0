...
cities = {
    'москва': ['1030494/7ac0ca5afc7043740b8b',
               '965417/865189726d0fbe08b9d4'],
    'нью-йорк': ['1030494/67a11a1c78844585d558',
                 '1030494/af47376e5f36430eb6d1'],
    'париж': ["1030494/034e640e963bfb8d57b6",
              '965417/59fd53058ec80f3849d6']
}
...
def guess_country(res, req):
    user_id = req['session']['user_id']
    city = sessionStorage[user_id]['guessed_cities'][-1]
    city_country = get_country(city)
    country = get_country_name(req)
    if country.lower() == city_country.lower():
        sessionStorage[user_id]['guess_country'] = True
        ...
        res['response']['text'] = f'Не угадали. Это {city_country}. Сыграем ещё?'
        res['response']['buttons'] = [{
            "title": "Покажи город на карте",
            "url": f"https://yandex.ru/maps/?mode=search&text={city.title()}",
            "hide": True
